...
export function validateMcpServerConfig(config: McpServerConfig): ValidationResult {
  const errors: ValidationError[] = [];
  ...
  if (config.oauth?.enabled) {
    if (config.oauth.scopes && config.oauth.scopes.some(scope => !scope.trim())) {
      errors.push({
        field: "oauth.scopes",
    ...
    if (config.oauth.authorizationUrl && !isValidUrl(config.oauth.authorizationUrl)) {
      errors.push({
...
export function validateMcpServerEntry(server: McpServerEntry): ValidationResult {
...
export function validateUniqueServerNames(servers: McpServerEntry[]): ValidationResult {
  const errors: ValidationError[] = [];
