...
fn read_project_metadata(root_sha: &str) -> Result<ProjectMetadata> {
    ...
    let content = std::fs::read_to_string(&path).context("Failed to read project metadata")?;
    Ok(serde_json::from_str::<ProjectMetadata>(&content)
...
fn write_project_metadata(sha256: &str, meta: &ProjectMetadata) -> Result<()> {
    ...
    std::fs::write(&tmp_path, content.as_bytes()).context("Failed to write project metadata")?;
    std::fs::rename(&tmp_path, &json_path).context("Failed to rename project metadata")?;
    Ok(())
}
...
mod tests {
    ...
    fn test_read_project_metadata_no_root() {
        ...
        if let Err(e) = result {
            assert_eq!(e.to_string(), "projects root not found");
        }
    }
    ...
    fn test_read_project_metadata_file_not_found() {
        ...
        if let Err(e) = result {
            assert_eq!(e.to_string(), "project.json not found");
        }
    }
